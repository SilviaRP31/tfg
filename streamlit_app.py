    ...
    if edad_input and motivo_input:
        ...
        if top_matches:
            ...
            for index, score in top_matches:
                st.write(f"ID Psicólogo: {df.iloc[index]['ID Psicólogo']} - Similitud: {score:.2f} "f"Previamete vinculado con un paciente estos motivos de: {df.iloc[index]['Motivo visita paciente']}")
        else:
